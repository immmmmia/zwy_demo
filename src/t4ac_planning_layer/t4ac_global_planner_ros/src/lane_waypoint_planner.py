...
class LaneWaypointPlanner(LaneGraphPlanner):
    ...
    def __init__(self, map_data, map_path, distance=3, map_flag=0):
        ...
        self.current_waypoint = None
        self.goal_xyz = None
        self.distance = distance    #s的间距
        self.previous_lane_id = None

        self.vehicle_id = None
        self.enter_time = None
        self.delta_time = 0.1   #路径中相邻waypoint间隔的时间尺度
        self.vehicle_speed = 0

        """
        ROS Subscribers
        """
        self.location_subscriber = rospy.Subscriber("/t4ac/localization/pose", Odometry, self.odometry_callback)
        self.goal_subscriber = rospy.Subscriber("/t4ac/planning/goal", PoseStamped, self.goal_callback)
        self.changeleft_subscriber = rospy.Subscriber("/t4ac/decision_making/lane_change_order_left",Bool, self.change_left)
        self.changeright_subscriber = rospy.Subscriber("/t4ac/decision_making/lane_change_order_right",Bool, self.change_right)

        self.enter_time_subscriber = rospy.Subscriber("/vehicle/time/enter",Float32, self.time_callback)
        self.speed_subscriber = rospy.Subscriber("/vehicle/speed",Int32, self.speed_callback)
        self.num_subscriber = rospy.Subscriber("/vehicle/num",Int32, self.num_callback)
    ...
    def time_callback(self, time):
        self.enter_time = time.data
    
    def speed_callback(self, speed):
        self.vehicle_speed = speed.data
    
    def num_callback(self, num):
        self.vehicle_id = num.data
        
    def vehicle_writer(self):
        file_name = "~/catkin_ws/src/t4ac_planning_layer/t4ac_global_planner_ros/src/csv/vehicle_" + str(self.vehicle_id).zfill(3)
        start = []
        goal = []
        id = [str(self.vehicle_id).zfill(3)]
        speed = [self.vehicle_speed]
        state = ["running"]
        start.append(self.current_xyz[0])
        start.append(self.current_xyz[1])
        goal.append(self.goal_xyz[0])
        goal.append(self.goal_xyz[1])     

        df1 = pd.DataFrame({'id':id,'speed':speed,'state':state})
        df2 = pd.DataFrame({'start':start,'goal':goal})
        # print(df1)
        # df2.to_csv(file_name)
        pd.concat([df1,df2],axis=1).to_csv(file_name, index=False, sep=',')
        print("保存了第",self.vehicle_id,"辆车的信息")
    
    def waypoints_writer(self, path_msg):

        file_name = "~/catkin_ws/src/t4ac_planning_layer/t4ac_global_planner_ros/src/csv/vehicle_path_" + str(self.vehicle_id).zfill(3)
        s = []
        x = []
        y = []
        z = []
        ...
        dataframe = pd.DataFrame({'s':s, 'x':x, 'y':y, 't':z})
        dataframe.to_csv(file_name, index=False, sep=',')
        print("保存了第",self.vehicle_id,"辆车的路径数据")
    ...
    def publish_waypoints(self, route):
        ...
        path_msg.header.seq = self.vehicle_id
        ...
        for waypoint in path_msg.waypoints:
            ...
            waypoint.transform.location.z = temp_z
            # wp_index += 1
            temp_z += self.delta_time * self.vehicle_speed
        
        # Print and publish waypoint route
        # print("\n" + tabulate(waypoints_data, headers=["waypoint", "x", "y", "z"]) + "\n")
        self.waypoint_publisher.publish(path_msg)
        self.vehicle_writer()   #记录vehicle成csv文件
        self.waypoints_writer(path_msg)     #记录path成csv文件
